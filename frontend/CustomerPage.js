...
const CustomersPage = () => {
    ...
    const fetchCustomers = async () => {
        setIsLoading(true);
        try {
            const response = await fetch('http://localhost:5000/api/account/get-customers-by-company', {
                method: 'POST',
    ...
    const handleAddCustomer = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await fetch('http://localhost:5000/api/account/add-customer', {
                method: 'POST',
                headers: { 
    ...
    const handleDeleteCustomer = async (customerId) => {
        ...
        if (!window.confirm('It will delete all the data associated with this customer. Do you confirm?')) return;
        setIsLoading(true);
        try {
            console.log('Deleting customer with ID:', customerId);
            const response = await fetch('http://localhost:5000/api/account/delete-customer', {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <div style={{
            ...
            {/* Add Customer Modal - Only shown for Admin */}
            ...
            {/* Edit Customer Modal - Only shown for Admin */}
            {userRole === 'Admin' && showEditModal && currentCustomer && (
                <div style={{
                    position: 'fixed',
                    top: 0,
                    left: 0,
